...
export default async function ServerPerformanceTest() {
  ...
  const PRIME_RANGE_END = 3_000_000; 

  const startTime = Date.now();

  await sleep(100); // Simulate some async delay (e.g., network, I/O) - this won't block the main thread.
  ...
  const totalDuration = `endTime: ${endTime} startTime: ${startTime}`;
  
  // const durationColor = totalDuration > 1500 ? 'text-red-500' : 
  //                       totalDuration > 500 ? 'text-yellow-600' : 'text-green-600';

  const durationColor = 'text-green-500'; // Since this is a blocking task, we expect it to be high.
